...
<div class="invoice-container">
    ...
    @if(!empty($jobcard->work_done))
        <div class="work-done-section">
            <h3><i class="fas fa-wrench"></i> Work Done</h3>
    ...
    <div class="invoice-table-section">
        ...
        <table class="invoice-table">
            ...
            <tbody>
                ...
                @endphp
                <!--<div style="color: red; font-weight: bold;">Mileage Rate (debug): {{ $mileageRate }}</div>
                -->
                <!-- Inventory Items -->
                ...
                <!-- Enhanced Labour Section with Hour Types Breakdown -->
                @php
                    ...
                    $totalWithExtras = $totalLabourCost + $callOutCost + $mileageCost;
                ...
                @if($normalHours > 0 || $overtimeHours > 0 || $weekendHours > 0 || $holidayHours > 0 || $callOutHours > 0 || $totalTravelKm > 0)
                    ...
                    <tr class="item-row">
                        <td class="item-desc">
                            ...
                            <br><small class="text-muted">{{ number_format($normalHours, 1) }} hours @ R{{ number_format($company->labour_rate ?? 750, 2) }}/hour</small>
                        </td>
                    ...
                    <tr class="item-row">
                        <td class="item-desc">
                            <strong>Professional Labour - Overtime Hours</strong>
                            <br><small class="text-muted">{{ number_format($overtimeHours, 1) }} hours @ R{{ number_format(($company->labour_rate ?? 750) * ($company->overtime_multiplier ?? 1.5), 2) }}/hour ({{ ($company->overtime_multiplier ?? 1.5) * 100 }}% rate)</small>
                        </td>
                        <td class="qty">{{ number_format($overtimeHours, 1) }}</td>
                        <td class="unit-price">R {{ number_format(($company->labour_rate ?? 750) * ($company->overtime_multiplier ?? 1.5), 2) }}</td>
                        <td class="total">R {{ number_format($overtimeCost, 2) }}</td>
                    </tr>
                    @endif
                    ...
                    <tr class="item-row">
                        <td class="item-desc">
                            <strong>Professional Labour - Weekend Hours</strong>
                            <br><small class="text-muted">{{ number_format($weekendHours, 1) }} hours @ R{{ number_format(($company->labour_rate ?? 750) * ($company->weekend_multiplier ?? 2.0), 2) }}/hour ({{ ($company->weekend_multiplier ?? 2.0) * 100 }}% rate)</small>
                        </td>
                        <td class="qty">{{ number_format($weekendHours, 1) }}</td>
                        <td class="unit-price">R {{ number_format(($company->labour_rate ?? 750) * ($company->weekend_multiplier ?? 2.0), 2) }}</td>
                        <td class="total">R {{ number_format($weekendCost, 2) }}</td>
                    </tr>
                    @endif
                    ...
                    <tr class="item-row">
                        <td class="item-desc">
                            ...
                            <br><small class="text-muted">{{ number_format($holidayHours, 1) }} hours @ R{{ number_format(($company->labour_rate ?? 750) * ($company->public_holiday_multiplier ?? 2.5), 2) }}/hour ({{ ($company->public_holiday_multiplier ?? 2.5) * 100 }}% rate)</small>
                        </td>
                    ...
                    <tr class="item-row">
                        <td class="item-desc">
                            <strong>Professional Labour - Call Out</strong>
                            <br><small class="text-muted">{{ number_format($callOutHours, 1) }} hours @ R{{ number_format($callOutRate, 2) }}/hour</small>
                        </td>
                        <td class="qty">{{ number_format($callOutHours, 1) }}</td>
                        <td class="unit-price">R {{ number_format($callOutRate, 2) }}</td>
                        <td class="total">R {{ number_format($callOutCost, 2) }}</td>
                    </tr>
                    @endif
                    ...
                    <tr class="item-row">
                        <td class="item-desc">
                            ...
                            <br><small class="text-muted">{{ number_format($totalTravelKm, 1) }}km @ R{{ number_format($mileageRate, 2) }}/km</small>
                        </td>
                        <td class="qty">{{ number_format($totalTravelKm, 1) }}</td>
                        <td class="unit-price">R {{ number_format($mileageRate, 2) }}</td>
                    ...
                    <tr class="subtotal-row">
                        <td colspan="3" class="subtotal-label">Labour Services Subtotal</td>
                        <td class="subtotal-amount">R {{ number_format($totalWithExtras, 2) }}</td>
                    </tr>
                    @endif
                @endif
